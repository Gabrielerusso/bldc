...
void mc_interface_set_current_rel(float val) {
	...
	static float drpm_dt = 0.0;
	static float current_rpm_factor = 1.0;

	if(cfg->max_erpm_rate > 1.0){
		float dticks = (float)(chVTTimeElapsedSinceX(last_time));
		//sample rpm at 40Hz
		if(dticks >= (float)(CH_CFG_ST_FREQUENCY/40)){
			float rpm_filtered = mc_interface_get_rpm();
			float drpm = rpm_filtered-last_rpm;
			drpm_dt = drpm/dticks; // d_rpm/d_ticks
			last_time = chVTGetSystemTimeX();
			last_rpm = rpm_filtered;
		}
		//convert rpm/s to rpm/ticks
		float rpm_dt_max_diff = cfg->max_erpm_rate/(float)CH_CFG_ST_FREQUENCY;
		float rpm_dt_thresold_diff = (rpm_dt_max_diff/4.0);
		...
		float cur_f = utils_map(drpm_dt, rpm_dt_thresold_diff, rpm_dt_max_diff, 1.0, 1.0/10.0);
		UTILS_LP_MOVING_AVG_APPROX(current_rpm_factor, cur_f, 30);
		current_rel *= current_rpm_factor;
	}
