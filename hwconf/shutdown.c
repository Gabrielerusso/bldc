...
#ifndef HW_HAS_BRIERU_POWERSWITCH //default vesc power switch
		// Private variables
...
#else	//BRIESC power switch version
	// Private variables
	static volatile float m_inactivity_time = 0.0;
	static THD_WORKING_AREA(shutdown_thread_wa, 128);
	static volatile bool m_init_done = false;
	static volatile uint64_t odometer_old = 0;
...
#endif //end if power switch selection (vesc/briesc)
